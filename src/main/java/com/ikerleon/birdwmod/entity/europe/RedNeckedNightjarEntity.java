...
public class RedNeckedNightjarEntity extends NocturnalBirdEntity {

    public RedNeckedNightjarEntity(EntityType<? extends AnimalEntity> type, World worldIn) {
        super(type, worldIn);
    }
    ...
    protected SoundEvent getAmbientSound() {
        if(!world.isDay() && this.isOnGround()  && !isSleeping()) {
    ...
    public static DefaultAttributeContainer.Builder createBirdAttributes() {
        return MobEntity.createMobAttributes().add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.0D).add(EntityAttributes.GENERIC_FLYING_SPEED, 0.70D).add(EntityAttributes.GENERIC_MAX_HEALTH, 5.0D);
    }
