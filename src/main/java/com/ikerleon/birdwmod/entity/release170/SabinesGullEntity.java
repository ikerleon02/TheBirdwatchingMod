...
public class SabinesGullEntity extends DiurnalBirdEntity {

    public SabinesGullEntity(EntityType<? extends AnimalEntity> type, World worldIn) {
        super(type, worldIn);
    }
    ...
    protected SoundEvent getAmbientSound() {
        if((this.onGround) && !isSleeping()) {
            return SoundHandler.SABINES_GULL_CALL;
        }
        else{
            return null;
        }
    ...
    public static DefaultAttributeContainer.Builder createBirdAttributes() {
        return MobEntity.createMobAttributes().add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.20D).add(EntityAttributes.GENERIC_FLYING_SPEED, 0.70D).add(EntityAttributes.GENERIC_MAX_HEALTH, 6.0D);
    }
    ...
    public PassiveEntity createChild(PassiveEntity mate) {
        return (SabinesGullEntity)this.getType().create(this.world);
    }
}
